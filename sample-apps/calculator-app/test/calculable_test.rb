...
module TestCalculable
  ...
  class TestInput < Minitest::Test
    ...
    def test_content_cant_start_with_a_multiplication_or_division_operator
      ...
      assert contents.all? { |c| assert_raises(InvalidContentError) { @input.content = c } }
    end
    ...
    def test_content_cant_contain_two_multiplication_or_division_operators_consecutively
      contents = %w( 1**2 1//2 1*/2 1/*2)
      assert contents.all? { |c| assert_raises(InvalidContentError) { @input.content = c } }
    end

    def test_content_cant_contain_any_operator_three_or_more_times_consecutively
      contents = %w( 1*++2 1++/2/ 1+*/2/ 1+--2/)
      assert contents.all? { |c| assert_raises(InvalidContentError) { @input.content = c } }
    end
